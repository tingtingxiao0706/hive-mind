...
| 策略 | 行为 | 适用场景 |
|------|------|---------|
| `progressive` (默认) | 关键词路由匹配后按需加载 | 通用场景 |
| `eager` | 启动时预加载所有技能 | 技能少、要求低延迟 |
| `lazy` | 跳过索引，显式指定技能直接加载 | 已知使用哪个技能 |
| `llm-routed` | LLM 从技能目录中自主选择并激活 | 技能多、用户意图模糊 |
...
传统的关键词/BM25 路由是词袋模型，对语义理解有限。`llm-routed` 策略将路由决策交给 LLM，同时保留引擎对激活和执行的完整控制：
...
Phase 2a: LLM 路由
  ┌─────────────────────────────────────────────────┐
  │  System Prompt 注入技能目录:                      │
  │  - translator: 翻译文本                          │
  │  - summarizer: 总结长文                          │
  │  - code-reviewer: 代码审查                       │
  │                                                 │
  │  LLM 调用 activate_skill("translator")           │
  │  → 引擎加载技能、校验安全、注册工具               │
  └─────────────────────────────────────────────────┘
               ↓
Phase 3: 执行
  ...
  │  第二次 LLM 调用，携带:                           │
  │  - translator 技能的完整指令                      │
  │  - run_script / read_resource 等工具              │
  │  → 执行任务并返回结果                             │
  └─────────────────────────────────────────────────┘
```

- 不需要技能时，LLM 直接回答（仅一次调用）
- 支持单次对话激活多个技能（多次 `activate_skill`）
- `activate_skill` 内含去重、数量限制等防护
...
const hive = createHiveMind({
  ...
  loading: {
    ...
    cacheSize: 50,
    catalogueTokenBudget: 2000, // llm-routed 模式下技能目录最大 token 预算
  },
